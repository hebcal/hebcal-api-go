...
// Point is a geographic coordinate used to bias autocomplete results toward
// the caller. It mirrors the shape of a geoip lookup result without depending
// on that package.
type Point struct {
	Latitude  float64
	Longitude float64
...
// AutoComplete generates typeahead results for qraw, biasing text-search
...
func (db *DB) AutoComplete(qraw string, near *Point) []Item {
...
func SortAutocomplete(values []Item, near *Point) {
...
func autocompleteScore(item Item, near *Point) float64 {
...
// location.
func (db *DB) geonameFulltextComplete(match string) []Item {
...
func (db *DB) zipFulltextComplete(match string) []Item {
...
func mergeZipGeo(zipMatches, geoMatches []Item) []Item {
	...
	m := make(map[string]Item, len(zipMatches)+len(geoMatches))
