...
func testServer(t *testing.T) (*Server, *httptest.Server) {
	...
	app.Now = func() model.GregDate {
		return model.GregDate{Year: 2026, Month: time.July, Day: 5}
	}
...
func TestSingleG2H(t *testing.T) {
	...
	if cc := resp.Header.Get("Cache-Control"); cc != httpx.CacheControl1Year {
...
func TestErrorMessages(t *testing.T) {
	...
	cases := []struct {
		...
		{"/converter?cfg=json&h2g=1&hy=5786&hm=0&hd=1", "bad monthName: 0"},
		{"/converter?cfg=json&h2g=1&hy=5786&hm=14&hd=1", "bad monthName: 14"},
		{"/converter?cfg=json&h2g=1&hy=5786&hm=Av&hd=31",
			"Hebrew day 31 out of valid range 1-30 for Av 5786"},
		{"/converter?cfg=json&h2g=1&hy=5785&hm=Adar2&hd=30",
			"Hebrew day 30 out of valid range 1-29 for Adar 5785"},
...
func TestETag304(t *testing.T) {
	...
	if resp2.Header.Get("Cache-Control") != httpx.CacheControl1Year {
...
func TestCSV(t *testing.T) {
	...
	if cc := resp.Header.Get("Cache-Control"); cc != httpx.CacheControl7Days {
		t.Errorf("Cache-Control = %q", cc)
	}
