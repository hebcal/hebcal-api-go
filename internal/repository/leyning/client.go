// Package leyning supplies Torah readings for the /shabbat API.
//
...
// and @hebcal/triennial. This package is the HTTP client for that endpoint
// plus the formatting that turns a reading into the classic-API "leyning"
// object, a port of formatLeyningResult() in @hebcal/rest-api and of the
// triennial block in hebcal-web src/myEventsToClassicApi.js.
//
...
// DefaultURL is the local hebcal-web /leyning endpoint.
const DefaultURL = "http://localhost:8080/leyning"

// cacheSize holds roughly a year of dates for both Israel and the
// Diaspora, so the working set of "this week" and "next week" always fits.
const cacheSize = 400

// requestTimeout bounds a single /leyning request. The endpoint is a local
// service and answers in single-digit milliseconds; this is a backstop.
const requestTimeout = 3 * time.Second

// Aliyah is one passage: book, begin and end chapter:verse.
type Aliyah struct {
	K string `json:"k"`
	B string `json:"b"`
	E string `json:"e"`
...
func (a Aliyah) short(showBook bool) string {
	prefix := ""
	if showBook {
		prefix = a.K + " "
	}
...
// Reading is one item of the /leyning?cfg=json response. Only the
...
type Reading struct {
	...
	Fullkriyah map[string]Aliyah `json:"fullkriyah"`
	...
	Triennial  map[string]Aliyah `json:"triennial"`
}

// hasDesc reports whether this reading was produced by the given event.
func (r *Reading) hasDesc(desc string) bool {
...
// Client fetches and caches readings from the /leyning endpoint.
type Client struct {
	url    string
	client *http.Client
	cache  *lru.Cache[cacheKey, []*Reading]
	group  singleflight.Group
}

// New returns a Client that fetches readings from baseURL.
func New(baseURL string) *Client {
	// lru.New only errors on a non-positive size, a programmer error here.
	cache, err := lru.New[cacheKey, []*Reading](cacheSize)
	if err != nil {
		panic(err)
	}
	return &Client{
...
func (c *Client) Readings(ctx context.Context, start, end model.GregDate, il bool) (map[string][]*Reading, error) {
	dates := isoDateRange(start, end)
	out := make(map[string][]*Reading, len(dates))
	var missing []string
	for _, d := range dates {
		if v, ok := c.cache.Get(cacheKey{d, il}); ok {
			out[d] = v
		} else {
	...
	for d, readings := range v.(map[string][]*Reading) {
		out[d] = readings
	}
	return out, nil
}
...
func (c *Client) fetch(ctx context.Context, from, to string, il bool) (map[string][]*Reading, error) {
	q := url.Values{}
	q.Set("cfg", "json")
	q.Set("start", from)
	q.Set("end", to)
	...
	byDate := make(map[string][]*Reading)
	...
	for _, d := range isoDateRange(fromD, toD) {
		c.cache.Add(cacheKey{d, il}, byDate[d])
	}
	return byDate, nil
}
...
func FindParshaReading(readings []*Reading) *Reading {
...
func FindHolidayReading(readings []*Reading, desc string) *Reading {
	for _, r := range readings {
		if r.hasDesc(desc) {
			return r
		}
...
func FormatLeyning(r *Reading, triennial bool) jsutil.OrderedObj {
	nums, hasMaftir, others := splitAliyotKeys(r.Fullkriyah)
	obj := make(jsutil.OrderedObj, 0, len(nums)+6)
	for _, k := range nums {
		obj = append(obj, jsutil.KV{Key: k, Val: r.Fullkriyah[k].WithBook()})
	}
	if r.Summary != "" {
		obj = append(obj, jsutil.KV{Key: "torah", Val: r.Summary})
	}
	if r.Haftara != "" {
		obj = append(obj, jsutil.KV{Key: "haftarah", Val: r.Haftara})
	}
	if r.Sephardic != "" {
		obj = append(obj, jsutil.KV{Key: "haftarah_sephardic", Val: r.Sephardic})
	}
	if len(r.Chabad) != 0 {
		obj = append(obj, jsutil.KV{Key: "haftarah_chabad", Val: r.Chabad.Summary()})
	}
	for _, k := range others {
		obj = append(obj, jsutil.KV{Key: k, Val: r.Fullkriyah[k].WithBook()})
	}
	if hasMaftir {
		obj = append(obj, jsutil.KV{Key: "maftir", Val: r.Fullkriyah["M"].WithBook()})
	}
	appendReasons(obj, r.Reason)
	if triennial && len(r.Triennial) != 0 {
		obj = append(obj, jsutil.KV{Key: "triennial", Val: formatTriennial(r.Triennial)})
	}
	return obj
}

// appendReasons suffixes " | <reason>" onto the aliyot and Haftarot
...
func appendReasons(obj jsutil.OrderedObj, reason map[string]string) {
	if len(reason) == 0 {
		return
	}
...
func formatTriennial(aliyot map[string]Aliyah) jsutil.OrderedObj {
	nums, hasMaftir, others := splitAliyotKeys(aliyot)
	obj := make(jsutil.OrderedObj, 0, len(nums)+1)
	for _, k := range nums {
		obj = append(obj, jsutil.KV{Key: k, Val: aliyot[k].WithBook()})
	}
	for _, k := range others {
		obj = append(obj, jsutil.KV{Key: k, Val: aliyot[k].WithBook()})
	}
	if hasMaftir {
		obj = append(obj, jsutil.KV{Key: "maftir", Val: aliyot["M"].WithBook()})
	}
	return obj
}
...
func splitAliyotKeys(aliyot map[string]Aliyah) (nums []string, hasMaftir bool, others []string) {
	for k := range aliyot {
		switch {
		case k == "M":
			hasMaftir = true
